...
// move it; the order is kept in local state only.
...
class _PlannerScreenState extends State<PlannerScreen> {
  final List<StudyBlock> _blocks = List.of(demoStudyBlocks);
  int _selectedDay = 1;

  static const _days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

  /// `onReorderItem` hands back a [newIndex] that already accounts for the
  /// row being lifted out, so no off-by-one correction is needed here.
      ...
      children: [
        ...
        Padding(
          ...
          child: GestureDetector(
            onTap: () => AppShellScope.of(context).go(ShellPage.exams),
            child: Container(
